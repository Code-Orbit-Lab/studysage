...
def test_chat_query_persists_both_turns(auth_headers, monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda query, sid: {
            ...
            "citations": [{"document_id": "x", "page": 1, "snippet": "..."}],
        },
    )
    ...
    messages = (
        ...
        .filter(ChatMessage.subject_id == uuid.UUID(subject_id))
...
def test_chat_history_returns_persisted_turns_in_order(auth_headers, monkeypatch):
    ...
    monkeypatch.setattr(
        chat_module,
        "query_ai_service",
        lambda q, sid: {"answer": "A1", "citations": []},
    )
    client.post(
        "/chat/query",
        json={"subject_id": subject_id, "message": "Q1"},
        headers=auth_headers,
    )
...
def test_quiz_submit_scores_and_persists_attempt(auth_headers, monkeypatch):
    ...
    r = client.post(
        f"/quiz/{quiz_id}/submit", json={"answers": answers}, headers=auth_headers
    )
    assert r.status_code == 200
    body = r.json()
    assert body["score"] == 1
    ...
    attempt = (
        ...
        .filter(QuizAttempt.quiz_id == uuid.UUID(str(quiz_id)))
        .first()
    )
    db.close()
...
def test_flashcards_generate_persists_cards(auth_headers, monkeypatch):
    ...
    db = SessionLocal()
    cards = (
        db.query(Flashcard).filter(Flashcard.subject_id == uuid.UUID(subject_id)).all()
    )
    db.close()
    assert len(cards) == 1
...
def test_planner_generate_persists_study_plan(auth_headers, monkeypatch):
    ...
    monkeypatch.setattr(
        planner_module,
        "generate_plan",
        lambda subjects, deadline, hours_per_day, start_date: {
            "plan": [{"day": 1, "date": "2026-08-01", "sessions": []}],
            "days_available": 5,
        },
    )
    r = client.post(
